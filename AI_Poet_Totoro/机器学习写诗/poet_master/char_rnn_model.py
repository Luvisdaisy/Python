...
SampleType = Enum(
    'SampleType', ('max_prob', 'weighted_sample', 'rhyme', 'select_given'))
    ...
    def __init__(self, is_training, batch_size, num_unrollings, vocab_size, w2v_model,
        ...
        if is_training and self.dropout > 0:
            cells = [tf.nn.rnn_cell.DropoutWrapper(
                cell, output_keep_prob=1.0-self.dropout) for cell in cells]

        multi_cell = tf.nn.rnn_cell.MultiRNNCell(cells)

        with tf.name_scope('initial_state'):
            self.zero_state = multi_cell.zero_state(
        ...
        if is_training:
            ...
            grads, _ = tf.clip_by_global_norm(tf.gradients(
                self.mean_loss, tvars), self.max_grad_norm)
        ...
        for step in range(epoch_size):
            ...
            if ((step+1) % freq == 0):
                logging.info('%.1f%%, step:%d, perplexity: %.3f, speed: %.0f words',
                             (step + 1) * 1.0 / epoch_size *
                             100, step, ppl_cumsum/(step+1),
